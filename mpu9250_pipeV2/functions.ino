...
void front(int sp, int sp1)
{
  digitalWrite(motL[0], 1);
  digitalWrite(motL[1], 0);
  analogWrite(motL[2], sp);
  digitalWrite(motR[0], 1);
  digitalWrite(motR[1], 0);
  analogWrite(motR[2], sp1);
}
void back(int sp,int sp1) {
  digitalWrite(motL[0], 0);
  digitalWrite(motL[1], 1);
  analogWrite(motL[2], sp);
  digitalWrite(motR[0], 0);
  digitalWrite(motR[1], 1);
  analogWrite(motR[2], sp1);
}
void brake() {
  digitalWrite(motL[0], 1);
  digitalWrite(motL[1], 1);
  analogWrite(motL[2], 0);
  digitalWrite(motR[0], 1);
  digitalWrite(motR[1], 1);
  analogWrite(motR[2], 0);
...
void mpu_data()
{
 //  while (Serial.available()) {
  ...
  pitch *= 180.0f / PI;
  yaw   *= 180.0f / PI;
  yaw   += 1.34; // Declination at Potheri, Chennai is 13 degrees 48 minutes and 47 seconds on 2017-16-04
  roll  *= 180.0f / PI;
}
