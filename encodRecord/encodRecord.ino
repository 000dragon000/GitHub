...
volatile  int encR_posH = 0;
volatile  int encL_posH = 0;
volatile int right_count=0;    /* Right wheel odometer */
...
void encR()
{
  if (digitalRead(encR_B)){ encR_pos++;encR_posH++;right_count++;}
  else {encR_pos--;encR_posH--;right_count--;}
}
void encL()
{
  if (digitalRead(encL_B)){ encL_pos++;encL_posH++;left_count++;}
  else {encL_pos--;encL_posH--;left_count--;}
}
//void encR()
//{
