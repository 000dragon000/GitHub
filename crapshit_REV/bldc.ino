...
void bldc_() {
  if (ps2_triangle && bldc_spd == 1500)///START
  {
    bldc_spd = 1550;
    bldc.write(bldc_spd);
  }

  if (ps2_triangle && bldc_spd != 1500) {//
    if (!ps2_square) bldc_spd += 10;
    else bldc_spd += 2;
    bldc.write(bldc_spd);
  }
  if (ps2_circle) {
...
void spin_bldc_() {
  if (ps2_left && spin_bldc_spd == 1500) {
   spin_bldc_spd = 1550;
    spin_bldc.write(spin_bldc_spd);
  }
  if (ps2_left && spin_bldc_spd != 1500) {
    if (!ps2_square) spin_bldc_spd += 10;
    else spin_bldc_spd += 2;
    spin_bldc.write(spin_bldc_spd);
  }
  if (ps2_right) {
    if (!ps2_square) spin_bldc_spd -= 10;
